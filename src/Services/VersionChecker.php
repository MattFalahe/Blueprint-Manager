...
/**
 * Compares the currently-installed Blueprint Manager version against the
 ...
 *
 * Resilience:
 *   - Result is cached for 6 hours so each Help-page visit doesn't hit
 *     Packagist.
 *   - HTTP request has a 3-second timeout.
 *   - Every failure path returns 'unknown' status so the Help page never
 *     errors out from a Packagist hiccup.
 *
 * Shared pattern across the plugin suite; the return shape matches so the
 * Blade renderer is copied verbatim between plugins.
 */
class VersionChecker
{
    /** Packagist v2 metadata URL for this package. */
    private const PACKAGIST_URL = 'https://repo.packagist.org/p2/mattfalahe/blueprint-manager.json';

    /** Composer package key under packages.* in the Packagist response. */
    private const PACKAGE_KEY = 'mattfalahe/blueprint-manager';

    /** Cache key for the fetched-latest result. */
    private const CACHE_KEY = 'blueprint-manager:packagist_latest';
    ...
    public function getStatus(): array
    {
        ...
        if ($isDevBranch) {
            return [
                ...
                'latest'         => $latest,
                'status'         => 'dev_branch',
                'message'        => 'You are running a development branch (' . $current . '), not a tagged release. The "Latest release" pill above is the most recent stable Packagist tag; your branch may be ahead of or behind it depending on local commits. Switch to a tagged version for a definitive "up to date" comparison.',
                'release_url'    => null,
            ];
        }
        ...
        if ($cmp < 0) {
            return [
                ...
                'release_url'    => 'https://github.com/MattFalahe/Blueprint-Manager/releases/tag/' . $latest,
            ];
        }
    ...
    protected function resolveInstalledVersion(): array
    {
        ...
        return [(string) config('blueprint-manager.version', '0.0.0'), 'config'];
    }
            ...
            try {
                $client = new Client([
                    ...
                    'headers'         => [
                        ...
                        'User-Agent' => 'SeAT-BlueprintManager/' . config('blueprint-manager.version', 'unknown'),
                    ],
                ]);
                ...
                if (! is_array($data) || ! isset($data['packages'][self::PACKAGE_KEY]) || ! is_array($data['packages'][self::PACKAGE_KEY])) {
                    Log::warning('[Blueprint Manager] VersionChecker: Packagist response missing expected packages.' . self::PACKAGE_KEY . ' shape');
                    return null;
                }
                ...
                Log::warning('[Blueprint Manager] VersionChecker: failed to fetch latest version from Packagist: ' . $e->getMessage());
                return null;
            }
        });
    }
