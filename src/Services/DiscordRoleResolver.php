 ...
 * for the webhook role-ping picker in Blueprint Manager settings.
 *
 ...
 * back to a plain manual-entry field.
 *
 * Ported from structure-manager/src/Services/DiscordRoleResolver.php so the
 * picker behaves identically across the suite.
 */
class DiscordRoleResolver
{
    ...
    public static function listRoles(): array
    {
        ...
        foreach ($providers as $provider) {
            try {
                ...
                Log::warning('[Blueprint Manager] DiscordRoleResolver: failed listing roles from ' . $provider . ': ' . $e->getMessage());
                continue;
            }
     ...
     *
     * The seat-discord-pings package is displayed as "SeAT Broadcast" in
     * user-facing UI per the project's canonical naming convention.
     */
    ...
    /**
     * Role lookup map keyed by Discord snowflake ID, for translating stored
     * raw role IDs back into human-readable names in the settings UI.
     *
